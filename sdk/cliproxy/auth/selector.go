...
//  2. Idempotency-Key from metadata
//  3. metadata.user_id (Claude Code format)
...
func ExtractSessionID(headers http.Header, payload []byte, metadata map[string]any) string {
	// 1. X-Session-ID header (explicit session binding)
	if headers != nil {
	...
	// 2. Idempotency-Key from metadata (set by handlers)
	if metadata != nil {
	...
	if userID != "" {
		...
		// Use full user_id if no session pattern but non-empty
		return "user:" + userID
	}

	// 4. conversation_id field (common in chat APIs)
	if convID := gjson.GetBytes(payload, "conversation_id").String(); convID != "" {
		return "conv:" + convID
	}

	// 5. Fallback: hash first few messages for stable session binding
