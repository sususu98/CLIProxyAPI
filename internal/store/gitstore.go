...
func (s *GitTokenStore) EnsureRepository() error {
	...
	if _, err := os.Stat(gitDir); errors.Is(err, fs.ErrNotExist) {
		...
		if errPull := worktree.Pull(&git.PullOptions{Auth: authMethod, RemoteName: "origin"}); errPull != nil && !errors.Is(errPull, git.NoErrAlreadyUpToDate) {
			if errors.Is(errPull, transport.ErrAuthenticationRequired) || errors.Is(errPull, plumbing.ErrReferenceNotFound) || errors.Is(errPull, transport.ErrEmptyRemoteRepository) {
